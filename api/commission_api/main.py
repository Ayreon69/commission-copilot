...
from .assistant.llm import LLMClient, LLMError, MistralClient
...
def create_app(settings: Settings | None = None, llm: LLMClient | None = None) -> FastAPI:
    ...
    if llm is None and settings.mistral_api_key:
        llm = MistralClient(settings.mistral_api_key, settings.mistral_model)
    assistant = None
    if llm is not None:
